defmodule FlyDeploy.ReloadScript do
  ...
  def replay_upgrade_startup(tarball_url, app) do
    ...
    beam_files = Enum.filter(all_beam_files, fn path ->
      String.contains?(path, "/#{app_version_dir}/ebin/")
    end)

    IO.puts("  Found #{length(beam_files)} app beam files to check (#{length(all_beam_files)} total)")
    ...
    IO.puts("  ✓ Copied #{copied_count} changed beam files (skipped #{length(beam_files) - copied_count} unchanged)")
